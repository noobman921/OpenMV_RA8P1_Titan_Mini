...
/* Forward declarations from qspi_operations.c */
extern fsp_err_t get_flash_status(void);
			...
			{				
                ...
                else
                {
                    ...
                    if ( RxByteBuffer[1] == ExpectedBlkNum )
                    {
                        ...
                        uint8_t aligned_buf[128] BSP_ALIGN_VARIABLE(8);
                        memcpy(aligned_buf, &RxByteBuffer[3], 128);
                        /* Split into 2 x 64-byte writes to match OSPI 64-byte combination function,
                         * with status polling between writes to avoid OSPI controller buffer corruption. */
                        err = R_OSPI_B_Write(&g_ospi0_ctrl, aligned_buf, (uint8_t *)Address, 64);
                        if (FSP_SUCCESS == err)
                        {
                        ...
                        if (FSP_SUCCESS == err)
                        {
                            err = R_OSPI_B_Write(&g_ospi0_ctrl, aligned_buf + 64, (uint8_t *)(Address + 64), 64);
                        }
                        if (FSP_SUCCESS == err)
                        {
                            err = get_flash_status();
