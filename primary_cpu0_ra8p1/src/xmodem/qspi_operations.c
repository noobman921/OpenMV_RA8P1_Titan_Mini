...
void deinit_qspi(const spi_flash_protocol_t spi_protocol_mode)
{
    ...
    if (SPI_FLASH_PROTOCOL_QPI == spi_protocol_mode)
    {
        ...
        error = R_OSPI_B_DirectWrite(&g_ospi0_ctrl, &data_exit_qpi, ONE_BYTE, false);
       ...
       /* close QSPI module */
    error = R_OSPI_B_Close(&g_ospi0_ctrl);
    if (FSP_SUCCESS != error)
    {
        while(1);
    }
...
void init_qspi(void)
{
    fsp_err_t err                                  = FSP_SUCCESS;
       ...
       err = R_OSPI_B_DirectWrite(&g_ospi0_ctrl, &(g_ospi0_cfg.write_enable_command), ONE_BYTE, false);
       if (FSP_SUCCESS != err)
       {
        ...
        */
       err = R_OSPI_B_DirectWrite(&g_ospi0_ctrl, data_sreg, SREG_SIZE, false);
       if (FSP_SUCCESS != err)
       {
